...
const statusConfig: Record<IdeaStatus, { idle: string; active: string }> = {
  DRAFT:       { idle: "bg-slate-100 text-[var(--fg-muted)] hover:bg-slate-200",       active: "bg-slate-600 text-white"   },
...
export function EditIdeaModal({ idea, locale, onClose }: EditIdeaModalProps) {
  ...
  return (
    ...
    >
      <div className="bg-[var(--bg-card)] rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[var(--border)]">
          <h2 className="text-lg font-semibold text-[var(--fg)]">{t("edit")}</h2>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          {/* Type */}
          <div>
            <p className="text-sm font-medium text-[var(--fg)] mb-2">{t("type")}</p>
          ...
          {/* Status */}
          <div>
            <p className="text-sm font-medium text-[var(--fg)] mb-2">{t("status")}</p>
