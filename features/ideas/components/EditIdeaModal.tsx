...
const statusConfig: Record<IdeaStatus, { idle: string; active: string }> = {
  DRAFT: {
    idle: "bg-slate-100 text-(--fg-muted) hover:bg-slate-200",
  ...
  ARCHIVED: {
    idle: "bg-amber-100 text-amber-700 hover:bg-amber-200",
...
export function EditIdeaModal({ idea, locale, onClose }: EditIdeaModalProps) {
  ...
  const [selectedType, setSelectedType] = useState<IdeaType>(
    idea.type ?? "PROJET",
  );
      ...
      <div className="bg-(--bg-card) rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        >
          ...
          <TagInput
            ...
            color={
              (ideaTypeConfig[selectedType] ?? ideaTypeConfig.PROJET).color
            }
          />
